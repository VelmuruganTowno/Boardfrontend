...
export default function AddPax(props) {
  ...
  const PreRoomRent = () => {
    ...
    axios.get(`${baseurl}bookingroomdetail/${uniqueId}/${id}`).then((res) => {
      ...
      if (moment(checkin).isBefore(checkout)) {
        roomInputs.forEach((element, index) => {
          if (element.id !== "") {
            axios
              .get(
                `${baseurl}getroomrent/${propertyId}/${element.roomType}/${StartDate}/${EndDate}`
              )
              .then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    roomInputs.forEach((item, index) => {
      axios
        .get(
          `${baseurl}getroomrent/${propertyId}/${item.roomType}/${StartDate}/${NewEndDate}`
        )
        .then((res) => {
          let Previous = _.filter(prevRoomRents, {
          ...
          if (Object.is(roomInputs.length - 1, index)) {
            ...
            axios.post(
              `${baseurl}bookingroomdetail/${uniqueId}/${bookingDetails.bookingId}`,
              calenderRents
            );
          }
        });
    ...
    if (checkingInclu == false) {
      axios.post(baseurl + "bookinginclusion", InclusionData);
    }
    // Room Lead data
