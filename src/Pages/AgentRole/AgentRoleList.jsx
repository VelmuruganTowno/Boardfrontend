...
function applySortFilter(array, comparator, query) {
    ...
    if (query) {
        return filter(
            array,
            (_agentRole) =>
                _agentRole.name.toLowerCase().indexOf(query.toLowerCase()) !== -1
        );
    }
    return stabilizedThis.map((el) => el[0]);
}

export default function AgentRoleList() {
    ...
    const [agentRoleLists, setAgentRoleLists] = useState([]);
    ...
    const downloadExcel = () => {
        const newData = agentRoleLists.map((row) => {
            delete row.id;
            delete row.uniqueId;
    ...
    const emptyRows =
        page > 0
            ? Math.max(0, (1 + page) * rowsPerPage - agentRoleLists.length)
            : 0;

    const filteredAgentRole = applySortFilter(
        agentRoleLists,
        getComparator(order, orderBy),
        filterName
    );

    const isNotFound = filteredAgentRole.length === 0;
                    ...
                    <TableContainer sx={{ minWidth: 499 }}>
                        <Table size="small">
                            <TableListHead
                                ...
                                rowCount={agentRoleLists.length}
