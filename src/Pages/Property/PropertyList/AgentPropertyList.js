...
function applySortFilter(array, comparator) {
  ...
  //     array,
  //     (_property) =>
  //       _property.hotelName.toLowerCase().indexOf(query.toLowerCase()) !== -1
  //   );
  // }
...
export default function AgentPropertyList() {
  ...
  useEffect(() => {
    let url = `/quickProperty/${uniqueid}`;
    Api.get(url).then((res) => {
      console.log("agentpropertylist|agent api data: ", res.data, url);
  ...
  const DownloadPdf = () => {
    ...
    doc.autoTable({
      body: propertyLists,
      columns: [
        { header: "Name", dataKey: "hotelName" },
        { header: "Location", dataKey: "city" },
        { header: "Star Rating", dataKey: "starCategory" },
        { header: "noofRooms", dataKey: "totalNoOfRooms" },
      ],
    });
      ...
      {width <= 768 ? (
            ...
            .map((row) => {
              ...
              return (
                <div key={id} className="propertycard">
                  ...
                  <div className="bookingContent">
                    ...
                    <div style={{ width: "50%", textAlign: "end" }}>
                      <button
                        ...
                        onClick={() => {
                          window.open(`/viewAgent/${propertyId}`,'_self');
                        }}
                      >
                        View
                      </button>
                  ...
                  <div className="bookingContent">
                    <p>
                      <span className="circle"></span>Conference Hall :{" "}
                      {jacuzzi ? "Yes" : "No"}
                    </p>
                    <p>
        ...
        <div style={{padding:'0 1%'}}>
          <TableListToolbar
            ...
            searchPlaceholderName={"Search Agent Property"}
            tableName={""}
            open={handleCreate}
            pdfDownload={DownloadPdf}
            downloadExcel={downloadExcel}
          />
          <Card style={{ boxShadow: 'none', border: '1px solid #F4F4F4',marginTop:'2px'}}>
                    ...
                    .map((row) => {
                      ...
                      return (
                        <TableRow hover key={id} tabIndex={-1}>
                          ...
                          <TableCell align="left" style={{fontSize:'12px'}}>{city}</TableCell>
                          <TableCell align="left" style={{fontSize:'12px'}}>{starCategory}</TableCell>
                          <TableCell align="left" style={{fontSize:'12px'}}>{totalNoOfRooms}</TableCell>
                          ...
                          <TableCell align="left" style={{fontSize:'12px'}}>
                            <Tooltip title="Edit">
                              <IconButton
                                aria-label="edit"
                                onClick={() => {
                                  window.open(`/addAgentProperty/${propertyId}`,'_self')
                                }}
                                style={{
                                  padding: "0px",
                                  color: "#F46D25",
