...
import DealoftheWeekForm from "./DealoftheWeekForm";
...
export default function DealoftheWeek() {
  ...
  const [weekDeal, setWeekDeal] = useState([]);
  ...
  const WeekDealAll = () => {
    Api.get("agentdealoftheweek").then((res) => {
      setWeekDeal(res.data);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - weekDeal.length) : 0;

  const filteredWeekDeal = applySortFilter(
    weekDeal,
    getComparator(order, orderBy),
    filterName
  );

  const isNotFound = filteredWeekDeal.length === 0;

  const Delete = (row) => {
    if (row.status == "1") {
      Api.delete(`agentdealoftheweek/${row.id}/${row.createdBy}`).then((res) => {
        if (res.status === 200) {
          WeekDealAll();
        }
      });
    } else {
      Api.put(`agentdealoftheweekactive/${row.id}/${row.createdBy}`).then((res) => {
        if (res.status === 200) {
          WeekDealAll();
        }
      });
