...
export default function NewBooking() {
  ...
  useEffect(() => {
    ...
    if (Object.keys(formError).length === 0 && isSubmit) {
      ...
      roomInputs.forEach((item) => {
        axios
          .get(
            `${baseurl}getroomrent/${propertyId}/${item.roomType}/${StartDate}/${EndDate}`
          )
          .then((res) => {
            const Name = {
  ...
  const AllHotel = async () => {
    await axios.get(baseurl + "propertyName").then((res) => {
      setHotelList(res.data);
    });
  };

  const AllCity = async () => {
    await axios.get(baseurl + "propertycityname/" + uniqueid).then((res) => {
  ...
  const handleChangeHotel = (data) => {
    ...
    axios.get(`${baseurl}propertydetails/${hotelId}`).then((res) => {
      setRoomInputs([
        {
          id: "",
          boardBasic: "",
  ...
  const handleChangeCity = (option) => {
    ...
    city = city.toString();
    axios.get(`${baseurl}propertycitynameidlist/${city}`).then((res) => {
      setHotelList(res.data);
    });
  };
  // Handle Night
  ...
  const OpenDialogView = () => {
    ...
    const previewData = {
      ...
      pendingAmount: `${
        parseFloat(townoPending) + parseFloat(hotelPendingAmount)
      }`,
      vacinated: checked,
      personInput,
      roomInputs,
  ...
  const WithoutMail = (e) => {
    ...
    axios.post(baseurl + "bookingdetails", submitData).then((res) => {
      ...
      if (
        ...
        axios.post(
          `${baseurl}bookingroomdetail/${uniqueid}/${res.data.bookingId}`,
          rent
        );
        // Room Inclusion Data
        ...
        // Room payemnt Data
        axios.post(baseurl + "roompayment", payment);
            ...
            <>
              <Formik enableReinitialize initialValues={initialValues}>
                <Form autoComplete="off">
                  <Grid container spacing={2}>
                    <Grid item lg={12}>
                      <h1 style={{ color: "#F46D25", margin: "0px" }}>
                        New Booking{" "}
                      </h1>
                    </Grid>
                    <Grid
